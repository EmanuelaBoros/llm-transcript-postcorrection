...
class TestAlignSentences(unittest.TestCase):
    ...
    def test_align_sentences(self):
        ...
        texts = text.split('\n')
        ocr_sentences = process_text(texts[0].replace('[OCR_toInput]', '').strip())
        gs_sentences = process_text(texts[1].replace('[ GS_aligned]', '').strip())
        ...
        # Check if the custom distance is smaller for aligned sentences than for non-aligned sentences
        for i, (ocr_sentence, gs_sentence) in enumerate(aligned_sentences):
            ...
            for j, other_gs_sentence in enumerate(gs_sentences):
                if j != i:
                    other_distance = custom_similarity(ocr_sentence, other_gs_sentence)
