...
def gather_and_archive_files(model_names, folder_path, output_folder):
    ...
    for run_type in ['few-shot', 'zero-shot']:
        ...
        for model_name in model_names:
            ...
            files_to_archive = glob.glob(os.path.join(folder_path, f"{model_name}_{run_type}*"))

            # Check if any files were found
            if files_to_archive:
                # Create an archive for the found files
                archive_name = os.path.join(output_folder, f"{model_name}_{run_type}.tar.gz")
            ...
            #     os.remove(file_path)
# Example usage:
model_names = ["ajmc-mixed", "ajmc-primary-text", "htrec", "icdar-2017", "icdar-2019", "impresso-nzz", "ina", "overproof"]
